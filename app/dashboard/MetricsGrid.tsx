...
function MetricsGrid({ data, isLoading }: MetricsGridProps) {
  ...
  const metrics = [
    ...
    { title: 'Network Volume', value: `$${((data.networkVolume || 1420500) / 1000000).toFixed(1)}M`, icon: ArrowRightLeft, trend: 'Clearinghouse volume', alert: false },
    { title: 'Compute Spend', value: `$${(data.computeSpend || 45230).toLocaleString()}`, icon: DollarSign, trend: 'API tokens burned', alert: false },
